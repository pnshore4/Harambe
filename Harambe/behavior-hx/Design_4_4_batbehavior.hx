	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				if((!(_attackdelay) && (((_position.getY() + 16) >= actor.getY()) && (actor.getY() >= (_position.getY() + -16)))))
				{
					trace("" + ("" + "\"before creating actor\""));
					createActorInNextScene(_SpitActor, actor.getXCenter(), actor.getYCenter(), Script.BACK);
